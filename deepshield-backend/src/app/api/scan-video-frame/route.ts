...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { frameBase64, mimeType, frameIndex, timestampSec, meta } = body ?? {};

    if (
      typeof frameBase64 !== "string" ||
      ...
      return NextResponse.json(
        apiError(
          "BAD_REQUEST",
          "frameBase64, mimeType, frameIndex, timestampSec are required",
          undefined,
          requestId,
        ),
        { status: 400 },
    ...
    const { base64: base64Raw, mimeTypeFromPrefix } = stripDataUrlPrefix(frameBase64);
    ...
    try {
      ...
      return NextResponse.json(
        apiError("BAD_REQUEST", "frameBase64 is not valid base64", undefined, requestId),
        { status: 400 },
      );
    }
