...
export function HeroPanel() {
  ...
  return (
    <GlassCard className="glass-card-tint-blue h-full p-6 md:p-8">
      ...
      <p className="mt-2 text-sm leading-relaxed text-ink/75">{t("heroPanelIntro")}</p>
        ...
        {ITEMS.map(({ key, href }) => (
          <li key={key}>
            {href ? (
              <Link
                href={href}
                className="flex items-start gap-2 rounded-xl bg-cream/70 px-3 py-2.5 text-sm text-ink/85 transition hover:bg-sage/35 hover:text-ink"
              >
                <span className="mt-0.5 text-sage">✓</span>
                <span>{t(key)}</span>
              </Link>
            ) : (
              <span className="flex items-start gap-2 rounded-xl bg-blue/25 px-3 py-2.5 text-sm text-ink/80">
                <span className="mt-0.5 text-sage">§</span>
                <span>{t(key)}</span>
              </span>
            )}
