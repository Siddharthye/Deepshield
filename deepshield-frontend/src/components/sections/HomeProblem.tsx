      ...
      <GlassCard className="bg-gradient-to-br from-blue/35 via-cream/40 to-sage/30 p-8 md:p-12">
        ...
        <ul className="mt-6 grid gap-2 sm:grid-cols-2">
          {POINT_KEYS.map((key) => (
            <li
              key={key}
              className="flex gap-2 rounded-xl bg-cream/65 px-3 py-2.5 text-sm text-ink/80 ring-1 ring-white/50"
            >
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-3">
          {STAT_KEYS.map(({ val, lbl, txt }) => (
            ...
            >
              ...
              <p className="mt-1 text-xs font-semibold uppercase tracking-wide text-ink/55">
                {t(lbl)}
              </p>
              <p className="mt-3 text-sm leading-relaxed text-ink/75">{t(txt)}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-10 rounded-2xl border border-sage/40 bg-blue/20 p-6 md:p-8">
          ...
          <ul className="mt-4 space-y-3">
            {WHAT_KEYS.map((key) => (
              <li key={key} className="flex gap-3 text-sm leading-relaxed text-ink/80">
